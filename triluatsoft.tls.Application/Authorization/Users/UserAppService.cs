...
namespace triluatsoft.tls.Authorization.Users
{
    ...
    public class UserAppService : tlsAppServiceBase, IUserAppService
    {
        ...
        private readonly triluatsoft.tls.OldTable.EmployeeAppService _employeeService;
        private readonly IRepository<triluatsoft.tls.OldTable.Employee> _employeeRepo;        
        ...
        public UserAppService(
            ...
            triluatsoft.tls.OldTable.EmployeeAppService employeeService,
            IRepository<triluatsoft.tls.OldTable.Employee> employeeRepo)
        {
        ...
        public async Task<GetUserForEditOutput> GetUserForEdit(NullableIdDto<long> input)
        {
            ...
            if (output.User.DateOfBirth != null)
            {
                output.User.Birthday = output.User.DateOfBirth.Value.Day + "/" + output.User.DateOfBirth.Value.Month + "/" +  output.User.DateOfBirth.Value.Year;
            }
        ...
        protected virtual async Task UpdateUserAsync(CreateOrUpdateUserInput input)
        {
            ...
            //var e = d.Month.ToString() + "/"+  d.Day.ToString()+ "/" + d.Year.ToString();

            //var ob = DateTime.Parse(input.User.Birthday);
            //user.DateOfBirth = date1;
            input.User.MapTo(user); //Passwords is not mapped (see mapping configuration)

            string[] dtsa = input.User.Birthday.Split("/");
            DateTime date1 = new DateTime(int.Parse(dtsa[2]), int.Parse(dtsa[1]), int.Parse(dtsa[0]));
            ...
            var emp = _employeeRepo.Get(user.EmployeeId);
            emp.Name = user.Name;
            emp.JobTitle = user.JobTitle;
            emp.JobPosition = user.JobPosition;
        ...
        protected virtual async Task CreateUserAsync(CreateOrUpdateUserInput input)
        {
            ...
            CultureInfo MyCultureInfo = new CultureInfo("vi-VN");
            DateTime obb1 = DateTime.ParseExact(input.User.Birthday, "dd/MM/yyyy", MyCultureInfo);
            //var ob = DateTime.Parse(input.User.Birthday);

            user.DateOfBirth = obb1;

            var emp = new triluatsoft.tls.OldTable.Employee();
            emp.Name = user.Name;
            emp.JobTitle = user.JobTitle;
            emp.JobPosition = user.JobPosition;
            emp.Address = user.Address;
